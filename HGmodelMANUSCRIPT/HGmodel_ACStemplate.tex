...
however, the influence of cholesterol on the lipid headgroup and glycerol backbone are still debated~\cite{huang99,simons04,somerharju09}.
...
parameter~\cite{brown78}. These effects are, however, so small that they are barely visible on the scale used in Fig.~\ref{ordPchol}.
...
\begin{figure}[]
  ...
  \caption{\label{ordPchol}
    ...
    In order to elucidate the relative change as a function of cholesterol content,
    the simulation results are shifted such that the (smaller) $S_\mathrm{CH}$
...
As already pointed out previously, the Berger-POPC-07/H\"oltje-CHOL-13 model
seriously exaggerates the effect of cholesterol on the phospholipid glycerol backbone and choline segments~\cite{ferreira13}.
In comprasion, the choline responses of both CHARMM36 and MacRog are in better qualitative agreement with experiments,
and also the glycerol backbone responses appear less drastic.
...
addition of cholesterol is the small decrease of gauche(-) and increase of gaughe(+) probability of the g$_3$-g$_2$-g$_1$-O(\textit{sn}-1) dihedral.
In conclusion, the results suggest that the significant effect of cholesterol on the lipid structures observed in simulations~\cite{zhu07,rog09,alwarawrah12} are overestimated by the model used in these studies, and the cholesterol only induces very small structural changes in the glycerol backbone.
