...
%DIF DEL ../../../nmrlipids.blogspot.fi/HGmodelMANUSCRIPT/HGmodel_ACStemplate.tex   Wed May 13 21:33:27 2015
%DIF ADD HGmodel_ACStemplate.tex                                                    Wed May 13 21:32:28 2015
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\DIFaddend \subsubsection{Phospholipid bilayer with low hydration level}
\DIFdelbegin \DIFdel{The experimental order parameters available in the literature~\mbox{%DIFAUXCMD
\cite{dvinskikh05b,ulrich94,bechinger91}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{Fig.~\ref{ordPhydr} shows the published~\mbox{%DIFAUXCMD
\cite{dvinskikh05b,ulrich94,bechinger91}
}%DIFAUXCMD
experimental order parameters
}\DIFaddend for the glycerol backbone and choline as a function of hydration level\DIFdelbegin \DIFdel{are shown in Fig.
~\ref{ordPhydr}. 
The independently reported values for the choline segments are in good agreement with each other (despite 
}\DIFdelend \DIFaddbegin \DIFadd{.
Despite }\DIFaddend slight differences in temperature and acyl chain composition\DIFdelbegin \DIFdel{),
and both }\DIFdelend \DIFaddbegin \DIFadd{,
the three independently reported data sets for the choline }\DIFaddend ($\beta$ and $\alpha$) \DIFdelbegin \DIFdel{show }\DIFdelend \DIFaddbegin \DIFadd{segments agree well with each other:
Both order parameters }\DIFaddend increase with decreasing hydration level.
\DIFdelbegin \DIFdel{It should be noted that only 
absolute values were measured in }\DIFdelend \DIFaddbegin \DIFadd{The glycerol backbone order parameters (g$_{3}$, g$_{2}$, g$_{1}$), in contrast,
have been observed~\mbox{%DIFAUXCMD
\cite{dvinskikh05b}
}%DIFAUXCMD
to slightly decrease with dehydration.
Note that }\DIFaddend the original experiments~\cite{dvinskikh05b,ulrich94,bechinger91} \DIFdelbegin \DIFdel{, but
we have }\DIFdelend \DIFaddbegin \DIFadd{measured only 
absolute values, but here
we }\DIFaddend included the signs measured in separate studies~\cite{hong95a,hong95b,gross97}. 
Consequently, the \DIFaddbegin \DIFadd{negative }\DIFaddend $\beta$ order parameter \DIFdelbegin \DIFdel{with negative sign }\DIFdelend actually increases with dehydration 
\DIFdelbegin \DIFdel{since the }\DIFdelend \DIFaddbegin \DIFadd{as its }\DIFaddend absolute value decreases~\cite{dvinskikh05b,ulrich94,bechinger91}.
\DIFdelbegin \DIFdel{Slight decrease for the glycerol backbone order parameters have been observed with dehydration~\mbox{%DIFAUXCMD
\cite{dvinskikh05b}
}%DIFAUXCMD
.
...
typically motivated by the  discussion \DIFdelbegin \DIFdel{about }\DIFdelend \DIFaddbegin \DIFadd{concerning }\DIFaddend the origin of the ``hydration repulsion''~\cite{israelachvili,israelachvili96,sparr11}.
\DIFdelbegin \DIFdel{However, the used simulation models are typically not compared }\DIFdelend \DIFaddbegin \DIFadd{Only one~\mbox{%DIFAUXCMD
\cite{mashl01}
}%DIFAUXCMD
of these studies, however, compared their simulation model }\DIFaddend to the experimental choline and glycerol backbone
order parameters\DIFdelbegin \DIFdel{(except by Mashl et al.
~\mbox{%DIFAUXCMD
\cite{mashl01}
}%DIFAUXCMD
).
In }\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend Fig.~\ref{ordPhydr} \DIFdelbegin \DIFdel{the glycerol backbone and choline order parameters are shown }\DIFdelend \DIFaddbegin \DIFadd{shows these order parameters }\DIFaddend as a function of hydration level for the CHARMM36, 
MacRog and GAFFlipid models (having the most realistic atomistic resolution structures) \DIFdelbegin \DIFdel{together with the Berger based }\DIFdelend \DIFaddbegin \DIFadd{and a Berger-based }\DIFaddend model 
(which is the most used lipid model)\DIFdelbegin \DIFdel{. 
To elucidate changes }\DIFdelend \DIFaddbegin \DIFadd{;
note that }\DIFaddend the simulation results \DIFdelbegin \DIFdel{are shifted to match better with experiments with full hydration.
The magnitude of shifting is written in the y-axis label.
The }\DIFdelend \DIFaddbegin \DIFadd{have been vertically shifted to ease the comparison with experimental response to dehydration.
Despite of some fluctuations, the }\DIFaddend increase of the choline \DIFdelbegin \DIFdel{order parameters with dehydration }\DIFdelend \DIFaddbegin \DIFadd{($\beta$ and $\alpha$) order parameters }\DIFaddend is seen in all \DIFdelbegin \DIFdel{models despite of some additional fluctuations. Thus }\DIFdelend \DIFaddbegin \DIFadd{four
models. The response of }\DIFaddend the choline order \DIFdelbegin \DIFdel{parameter response }\DIFdelend \DIFaddbegin \DIFadd{parameters }\DIFaddend to dehydration can\DIFdelbegin \DIFdel{be
intepreted to be in qualitative agreement }\DIFdelend \DIFaddbegin \DIFadd{, therefore, be
interpreted to qualitatively agree }\DIFaddend with experiments.
The situation is significantly more complicated for the glycerol backbone\DIFdelbegin \DIFdel{segments 
and none of the models can be interpreted to qualitatively agree with experiments
for all the segmets}\DIFdelend \DIFaddbegin \DIFadd{: 
None of the four models produced the experimentally seen trends
in all the (g$_{3}$, g$_{2}$, g$_{1}$) segments}\DIFaddend .

The qualitative agreement \DIFdelbegin \DIFdel{with experiments in all simulation models for }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend the $\alpha$ and $\beta$ order parameters \DIFdelbegin \DIFdel{as a function of hydration indicates thatthe }\DIFdelend \DIFaddbegin \DIFadd{with experiments in all four simulation models (Fig.~\ref{ordPhydr}) indicates that,
despite the unrealistic structures at full hydration (Figs.~\ref{HGorderparameters} and~\ref{FullHydrationComparisonTable}),
the }\DIFaddend structural response of the choline headgroup to dehydration is somewhat realistic\DIFdelbegin \DIFdel{despite the unrealistic structures at full hydration.
The most }\DIFdelend \DIFaddbegin \DIFadd{.
A }\DIFaddend likely explanation is that \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{as the interlamellar space shrinks with dehydration, the whole }\DIFaddend choline group
orients more parallel to the membrane\DIFdelbegin \DIFdel{plane with dehydration due to restricted interlamellar space}\DIFdelend .
Indeed, \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{upon dehydration the angle between }\DIFaddend P--N (phoshate phosphorus to choline nitrogen) vector \DIFdelbegin \DIFdel{angle with membrane normal shows an increase for
all models as a function of dehydration }\DIFdelend \DIFaddbegin \DIFadd{and membrane normal increases for
all the four models }\DIFaddend (Fig.~\ref{PNangle}). However,
the amount of increase depends on the model. Especially the DLPC simulations with Berger model
predict significantly stronger P--N vector tilt \DIFdelbegin \DIFdel{compared with }\DIFdelend \DIFaddbegin \DIFadd{than }\DIFaddend the other models. The Berger model
has also generally larger P--N vector angles and its choline order parameters are more off from
experiments than the other three models \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{(Fig.~\ref{HGorderparameters2}). }\DIFaddend Thus the relatively modest tilting with dehydration
predicted by MacRog, CHARMM36 and GAFFlipid is probably more realistic.

It \DIFdelbegin \DIFdel{should be also noted that the free energy landscape is not correct }\DIFdelend \DIFaddbegin \DIFadd{must be stressed that }\DIFaddend in the models which
\DIFdelbegin \DIFdel{are not able to reproduce }\DIFdelend \DIFaddbegin \DIFadd{incapable of reproducing }\DIFaddend the experimental order parameters \DIFdelbegin \DIFdel{. Thus the energetical response
to the dehydration may not be correct}\DIFdelend \DIFaddbegin \DIFadd{the free energy landscape is not correct. Thus
}\DIFaddend even though the order parameter response \DIFaddbegin \DIFadd{to dehydration }\DIFaddend is qualitatively correct\DIFdelbegin \DIFdel{.
This issue }\DIFdelend \DIFaddbegin \DIFadd{,
the energetic response is likely to be incorrect.
This }\DIFaddend may have some influence on \DIFdelbegin \DIFdel{the }\DIFdelend dehydration energetic calculations made \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{using }\DIFaddend the Berger model~\cite{eun09,schneck12}.
\begin{figure}[]
  \centering
...
The response of the glycerol backbone \DIFdelbegin \DIFdel{to dehydration }\DIFdelend seems to be more subtle than that of the choline headgroup\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{;
}\DIFaddend none of the \DIFaddbegin \DIFadd{four }\DIFaddend models reproduced the experimental \DIFdelbegin \DIFdel{results with the accuracy allowing
the }\DIFdelend \DIFaddbegin \DIFadd{trends upon dehydration with enough accuracy to invite a
}\DIFaddend structural interpretation.
